...
- `rush_mode` (bool) — behavior deliberately deferred; implemented as a swappable `SETUP_STRATEGIES` entry (`SnakeDraftSetup` vs. a stub `RushModeSetup`) so the real rules can be dropped in later without restructuring.
